...
async function create(req, res) {
    try {
        const data = await TransactionService.create(req.body, res.locals.user);
...
async function updateOne(req, res) {
    try {
        const data = await TransactionService.updateOne(req.params.id, req.body, res.locals.user);
        return response_json_1.responseSuccess(res, data, 200);
    }
    catch (error) {
        logger_mixed_1.logger.error(error);
